...
def algo_config(env_id: str, policies, policies_to_train):
    cfg = (
        ...
        .training(
            ...
            double_q=USE_DOUBLE_Q,          # improvement
            dueling=False,
...
def training(checkpoint_path: str, max_iterations: int = 5000):
    ...
    meta = {
        ...
        "model": "MLP(256,256)",          # <-- updated label to match config
        "double_q": USE_DOUBLE_Q,
        "dueling": False,
        ...
        "gamma": 0.99,
        "target_network_update_freq": 1000,
        "epsilon_schedule": "default (removed custom decay)",
    }
    logger = RunLogger(
        base_dir=str(Path("results").resolve()),
        global_seed=GLOBAL_SEED,
        algo_name="DQN_D1_DOUBLE_NSTEP",
        meta=meta,
    )
